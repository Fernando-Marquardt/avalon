...
if (isset($_SESSION["user"][0], $_GET["town"])) {
    ...
    if ($prod[0] - $town[3] - $town[12] < 5) {
        $prod[0] = $town[3] + $town[12] + 5; //noob protection against negative crop production values
    }
} else {
    header('Location: login.php');
    die();
...
if (!isset($_GET["v"])) {
    ...
    echo "<div style='position:relative; margin: 0 auto; width: 640; top:0; cursor:url({$imgs}{$fimgs}cursor.png), auto;'><img src='{$imgs}{$fimgs}x.gif'>
        ...
        <img src='{$imgs}{$fimgs}back.jpg' width='640' height='372' style='position:absolute; left:0; top:75;'>";
         ...
         if ($data[7])
            if ($data[7] == 10) echo "<img src='{$imgs}{$fimgs}b22.png' style='position:absolute; left:328; top:165;' name='hall'>";
            else echo "<img src='{$imgs}{$fimgs}b7.png' style='position:absolute; left:335; top:170;' name='hall'>";
        ...
        echo "<img src='{$imgs}{$fimgs}border2.jpg' width='640' height='35' style='position:absolute; left:0; top:445;'>
         ...
         echo "<span style=\"position:absolute; left:0; top:0; height:75; width:640;\">
            <img src='{$imgs}{$fimgs}00.gif' title='{$lang['crop']}'><span id=\"res0\"></span>/{$lim[0]}
            <img src='{$imgs}{$fimgs}01.gif' title='{$lang['lumber']}'><span id=\"res1\"></span>/{$lim[1]}
            <img src='{$imgs}{$fimgs}02.gif' title='{$lang['stone']}'><span id=\"res2\"></span>/{$lim[1]}
            <img src='{$imgs}{$fimgs}03.gif' title='{$lang['iron']}'><span id=\"res3\"></span>/{$lim[1]}
            <img src='{$imgs}{$fimgs}04.gif' title='{$lang['gold']}'><span id=\"res4\"></span>/{$lim[2]} <br>
            <img src='{$imgs}{$fimgs}house_.gif' title='Upkeep'>" . ($town[3] + $town[12]) . "/{$lim[3]}
            <img src='{$imgs}{$fimgs}morale_.gif' title='Morale'>{$town[5]}%.<br>
            Production: <img src='{$imgs}{$fimgs}00.gif' title='{$lang['crop']}'>" . ($prod[0] - $town[3] - $town[12]) . "/h
            ...
            <img src='{$imgs}{$fimgs}04.gif' title='{$lang['gold']}'>{$prod[4]}/h</span>";
        ...
        <map name='Map'>";
           if ($data[0]) {
               ...
               echo "<area shape='rect' coords='60,52,115,120' style='cursor:url({$imgs}{$fimgs}cursor.png), auto;' href='gmill.php?town={$town[0]}' onmouseout=\"gmill.src='{$imgs}{$fimgs}b0.png', showtext('-')\" onmouseover=\"gmill.src='{$imgs}{$fimgs}b0_.png', showtext('".$name[0]."')\">";}
