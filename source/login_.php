...
if (isset($_POST["name"], $_POST["pass"])) {
    ...
    if ((!$config[2][1]) && ($_SESSION["user"][4] < 4)) {
        $_SESSION = array();
        session_destroy();
        msg($lang['loginClosed'], 'warning');
    } else if ($_SESSION["user"][0]) {
        if (check_d($_SESSION["user"][0])) {
            $row = update_lastVisit($_SESSION["user"][0]);
            msg("{$lang['welcome']}, {$_SESSION["user"][1]}.</br>{$lang['youHave']} {$row[0]} {$lang['newRep']} {$row[1]} {$lang['newMsg']}.", 'success');
        } else {
            header('Location: logout.php');
        }
    } else {
        msg($lang['noUserWrong'], 'warning');
    }
} else {
    msg($lang['noInput'], 'warning');
}
?>
